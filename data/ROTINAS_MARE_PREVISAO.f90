...
!C	Transformar variaveis passadas para a subrotina
	...
	decode (2, '(i2)', df ) dia_fim
	decode (2, '(i2)', mf ) mes_fim
	decode (4, '(i4)', af ) ano_fim
	decode (10, '(f10.2)', nivel ) s0
	SOMET=S0/100.
...
!c	fuso_horario			= fuso_horario char*4	= fuso char*5
 ...
 6000	format (i3,18a)
	...
	decode (10, '(f10.2)', nivel_medio ) s0_anterior
...
!c---------------------------------------------------------------------------
	...
	&				VETMIN (J), VETALT (J), J = 1, IND)
			...
			ENCODE (2,'(I2)', VETDIA (IND) ) KTDIA (I)
			ENCODE (2,'(I2)', VETHOR (IND) ) KTHOR (I)
			ENCODE (2,'(I2)', VETMIN (IND) ) KTMIN (I)
			AATAB = (ATAB (I) /10.) / 10.
			ENCODE (4,'(F4.1)', VETALT (IND) ) AATAB
...
3303	format ('3',i4,i2,i2,i2,9(a2,a2,a4))
	...
	if (sim .eq. 'S') then
		if (tipo .eq. '2') then
			close (unit = 22, dispose = 'delete')
			close (unit = 33, dispose = 'delete')
		else
			if (tipo .eq. '1') then
				close (unit = 22, dispose = 'delete')
			else
...
!C				      	write (22,2202) (iwah (l1),l1=1,24)
				DO K = 1,24
					IHORA = K - 1
					ENCODE (2, '(I2)', AHORA) IHORA
					IF (K .LE. 10) AHORA (1:1) = '0'
					ENCODE (4, '(I4)', AIWAH ) IWAH (K)

!C					TRANFORMAR A DATA EM ALFA PARA GRAVACAO
					ENCODE (2, '(I2)', DIA_S ) IND2
					IF (IND2 .LT. 10) DIA_S (1:1) = '0'
					ENCODE (2, '(I2)', MES_S) (IND_MES - 1)
					IF ((IND_MES - 1).LT.10) MES_S(1:1)='0'
					ENCODE (4,'(I4)', ANO_S) KANO
	...
	do i = 1, index
		...
		if (ktmes (i) .ge. mes_ini .and. ktmes (i) .le. mes_fim) then
			if (ktdia (i) .ge. dia_ini .and. ktdia (i) .le. dia_fim) then
				...
				ENCODE (4,'(I4)', VETano  ) KTANO (I)
				ENCODE (2,'(I2)', VETmes  ) KTMES (I)
				ENCODE (2,'(I2)', VETDIA  ) KTDIA (I)
				ENCODE (2,'(I2)', VETHOR  ) KTHOR (I)
				ENCODE (2,'(I2)', VETMIN  ) KTMIN (I)
				AATAB = (ATAB (I) /10.) / 10.
				ENCODE (4,'(F4.1)', VETALT  ) AATAB
...
!c	concatenacao de variaveis e trasformacao de variaveis
	...
	fuso = fuso_horario (1:3) // '.' // fuso_horario (4:4)
	decode (10,'(f10.2)', nivel_medio ) s0
	SOMET = S0 / 100.

!c------------------------------------------------------------------------------
!C	GRAVACAO DO 1 ARQUIVO

	ENCODE (3, '(I3)', M1_ALFA) M1
...
!C	GRAVACAO DOS REGISTROS DO 2 ARQUIVO
	...
	DO INDICE = 1, MADEX
		ENCODE (4, '(I4)' , HORA_ALFA)  KTHOR (INDICE)
		ENCODE (4, '(I4)' , MIN_ALFA)   KTMIN (INDICE)
		ENCODE (2, '(I2)' , DIA_ALFA)   KTDIA (INDICE)
		ENCODE (2, '(I2)' , MES_ALFA)   KTMES (INDICE)
		ENCODE (4, '(F4.1)', ALT1)	ATAB  (INDICE)
...
!C	TRNSFORMAR OS MESES DA IMPRESSAO PARA CHARACTER
	...
	encode (2,'(i2)', mes_semana1) m
	encode (2,'(i2)', mes_semana2) m2
	encode (2,'(i2)', mes_semana3) m3
...
9403	encode (4, '(i4)' , hora1) kthor (indi1)
	encode (4, '(i4)' , minu1) ktmin (indi1)
	encode (4, '(f4.1)' , alt1) atab (indi1)
...
9406	encode (4, '(i4)' , hora1) kthor (indi2)
	encode (4, '(i4)' , minu1) ktmin (indi2)
	encode (4, '(f4.1)' , alt1) atab (indi2)
	write (13,302) br3,hora1,minu1,alt1
...
9412	encode (4, '(i4)' , hora1) kthor (indi3)
	encode (4, '(i4)' , minu1) ktmin (indi3)
	encode (4, '(f4.1)' , alt1) atab (indi3)
	write (13,302) br3,hora1,minu1,alt1
...
9415	encode (4, '(i4)' , hora1) kthor (indi4)
	encode (4, '(i4)' , minu1) ktmin (indi4)
	encode (4, '(f4.1)' , alt1) atab (indi4)
	write (13,302) br3,hora1,minu1,alt1
...
9422	encode (4, '(i4)' , hora1) kthor (indi5)
	encode (4, '(i4)' , minu1) ktmin (indi5)
	encode (4, '(f4.1)' , alt1) atab (indi5)
	write (13,302) br3,hora1,minu1,alt1
...
9426	encode (4, '(i4)'  , hora1) kthor (indi6)
	encode (4, '(i4)'  , minu1) ktmin (indi6)
	encode (4, '(f4.1)' , alt1) atab (indi6)
	write (13,302) br3,hora1,minu1,alt1
...
!c	calculo dos dias da linha
	...
	encode (2,'(i2)', dia1) idia1
	encode (2,'(i2)', dia16) idia16
...
!c	semana. Cada programa usara as letras necessarias.
	...
	decode (2,'(f2.0)', dia) d1
	decode (2,'(f2.0)', mes) m1
	decode (4,'(f4.0)', ano) a1
